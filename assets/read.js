(async function () {
    ...
    const EMV_AID2NAME = [
        ...
        ['A000000065', 'JCB'],
        ['A000000324', 'Discover'],
        ['A000000790', 'AMEXChina'], // A00000000790010602 for AMEXChinaDebit, A00000000790010502 for AMEXChinaCredit
    ];
    const PBOC_TTI2NAME = {
        '01': 'Load',
        '02': 'Load',
                ...
                for (let i = 0; i < log_format.length; i++) {
                    ...
                    switch (tag) {
                        ...
                        case 0x9F4E:
                            ...
                            break;
                        case 0x9C:
                            item['type'] = ISO8583_ProcessingCode2Name[extractField()] ?? 'Unknown:' + extractField();
                            break;
                        case 0x9F36:
    ...
    let ReadPPSE = async (fci) => {
        ...
        if (AppLabel) {
            AppLabelString = GBKDecoder.decode(AppLabel).toLowerCase();
    ...
    let ReadAnyCard = async (tag) => {
        ...
        if (tag.type === "felica" && tag.systemCode === "8008") {
            ...
            if (r.endsWith('9000')) {
                ...
                // Put it here because after select AMTJAVACARD some other AID select commands will fail to run on this specific card.
